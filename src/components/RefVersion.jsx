...
export default function RefVersion() {
  const [isModalOpen, setModalOpen] = useState(false); // Controls modal visibility
  const productToDelete = useRef(''); // Holds the product ID across renders
  ...
  function handleDelete(productId) {
    productToDelete.current = productId; // Store the ID without re-rendering
    console.log('Select product to delete:', productId);
    setModalOpen(true); // Open the modal
  }

  function confirmDelete() {
    console.log('Deleting product:', productToDelete.current);
  ...
  return (
    <div>
      <h2>Product List With useRef Version</h2>
